...
def read_cycle(cycle_id: int, db: Session = Depends(get_db)):
...
def list_cycles(station_name: Optional[str] = None, db: Session = Depends(get_db)):
...
def update_cycle(cycle_id: int, update: schemas.CycleUpdate, db: Session = Depends(get_db)):
    db_cycle = db.query(models.Cycle).filter(models.Cycle.id == cycle_id).first()
    if db_cycle is None:
        raise HTTPException(status_code=404, detail="Cycle not found")
